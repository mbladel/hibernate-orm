...
milvus() {
    ...
    temp_dir=$(mktemp -d)
    cat <<EOF >${temp_dir}/embedEtcd.yaml
...
EOF
    ...
    cat << EOF > ${temp_dir}/user.yaml
# Extra config to override default milvus.yaml
EOF
    $CONTAINER_CLI rm -f milvus || true
    $CONTAINER_CLI run -d \
          --name milvus \
          ...
          -v ${temp_dir}//embedEtcd.yaml:/milvus/configs/embedEtcd.yaml \
          -v ${temp_dir}//user.yaml:/milvus/configs/user.yaml \
          -p 19530:19530 \
          -p 9091:9091 \
          -p 2379:2379 \
...
neo4j_2025_4() {
  ...
  $CONTAINER_CLI run --name neo4j -p7474:7474 -p7687:7687 -e NEO4J_AUTH=neo4j/hibernate_orm_test -d ${DB_IMAGE_NEO4J_2025_4:-docker.io/neo4j:2025.04.0}
