...
| `timestampadd()` | Add an interval to a datetime | `timestampadd(field, interval, datetime)` | ✖
| `timestampdiff()` | Extract the interval between two datetimes | `timestampdiff(field, from, to)` | ✖
|===
...
[discrete]
===== Adding an interval to a date or time type

This function adds an `interval` number of the unit of time specified by the `field` parameter to a date, time, or datetime expression.

- Its first argument is a date/time field type, specifying the temporal units the interval will be expressed in.
- Its second argument is an expression that evaluates to an integral number, representing the interval to be added.
- Its third argument is an expression that evaluates to a date, time, or datetime that will be the target of the addition.

The syntax is `timestampadd(field, interval, datetime)`.

The resulting date, time or datetime will be of the same type as the `datetime` expression parameter.

Also, `dateadd()` can be used as a synonym of this function.

[[function-timestampdiff]]
[discrete]
===== Extracting the interval between two date or time types

This function extracts the number of temporal units, specified by the `field` parameter, which represents the interval between the `from` and `to` date, time, or datetime expression parameters.

- Its first argument is a date/time field type, specifying the temporal units the resulting interval will be expressed in.
- Its second argument is an expression that evaluates to a date, time, or datetime, representing the start of the resulting interval.
- Its third argument is an expression that evaluates to a date, time, or datetime, representing the end of the resulting interval.

The syntax is `timestampdiff(field, from, to)`.

Also, `datediff()` can be used as a synonym of this function.
