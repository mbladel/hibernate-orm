...
| `type()` | The (concrete) type | `type(e)` | &check;
...
The function `type()`, applied to an identification variable or entity/embeddable valued path expressions, evaluates to the concrete type, that is, the Java `Class`, of the referenced entity/embeddable.
