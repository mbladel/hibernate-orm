...
This function truncates a date, time, or datetime to the specified field.

The syntax is `truncate(datetime, field)`. Supported fields are: `year`, `month`, `day`, `hour`, `minute` or `second`.

Truncating a date, time or datetime value translates to obtaining a result of the same type for which all the values of temporal units smaller than `field`
have been pruned. For hours, minutes and second this means setting them to `00`. For months and days, this means setting them to `01'.
