...
ORM 6.6 introduced support for `@Embeddable` type inheritance. With it, we also allowed the `type()` and `treat()` functions to work with embeddable-typed paths as well. As a consequence, the `SqmTreatedPath#getTreatTarget()` method will now return a generic `ManagedDomainType`, which could in turn be an `EntityDomainType` (as it was before) or also an `EmbeddableDomainType` instance.