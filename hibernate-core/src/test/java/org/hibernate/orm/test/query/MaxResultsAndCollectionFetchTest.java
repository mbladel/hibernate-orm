...
public class MaxResultsAndCollectionFetchTest {
	...
	public void testEntityGraph(SessionFactoryScope scope) {
		...
		scope.inTransaction( session -> {
			...
			final Author author = session
					.createSelectionQuery( "from Author a order by a.name", Author.class )
					.setMaxResults( 1 )
	...
	public void testJoinFetch(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			final Author author = session
					.createSelectionQuery( "from Author a left join fetch a.books order by a.name", Author.class )
					.setMaxResults( 1 )
					.getSingleResult();
			assertThat( Hibernate.isInitialized( author.getBooks() ) ).isTrue();
			assertThat( author.getBooks() ).hasSize( 2 );
