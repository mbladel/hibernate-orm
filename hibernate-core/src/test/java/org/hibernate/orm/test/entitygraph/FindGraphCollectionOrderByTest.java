...
public class FindGraphCollectionOrderByTest {
	...
	private void executeTest(SessionFactoryScope scope, String hint, boolean find) {
		scope.inTransaction( session -> {
			...
			long i = 1;
			for ( final Level2 child : root.getChilds() ) {
				if ( i == 2 ) {
					assertThat( child.getChilds() ).matches( Hibernate::isInitialized ).hasSize( 1 );
				}
				assertThat( child.getId() ).as( "Childs not in expected order" ).isEqualTo( i++ );
			}
		} );
	}
