...
import org.hibernate.orm.test.where.annotations.MutationQueriesWhereTest;
...
/**
 * Same as {@link MutationQueriesWhereTest},
 * but using {@link Filter @Filter}
 *
 * @author Marco Belladelli
 */
@SessionFactory( useCollectingStatementInspector = true )
@DomainModel( annotatedClasses = {
		MutationQueriesFilterTest.BaseEntity.class,
...
public class MutationQueriesFilterTest {
	...
	public void testUpdateWithCondition(SessionFactoryScope scope) {
		...
		scope.inTransaction( session -> {
			session.enableFilter( "deleted_filter" );
			final MutationQuery mutationQuery = session.createMutationQuery(
					"UPDATE UserEntity SET name = 'Marco' where id = :id" );
			mutationQuery.setParameter( "id", 1L );
			mutationQuery.executeUpdate();
