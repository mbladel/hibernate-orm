...
public class IdClassBackrefTest {
	...
	public void testInitializedLines(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			...
			assertThat( item100.getLines() ).hasSize( 1 );
			final Item item = item100.getLines().get( 0 ).getItem();
	...
	public void testFindABucket(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			final Bucket bucket = session.find( Bucket.class, new PkBucket().withId( 200L ) );
			assertThat( bucket.getLines() ).hasSize( 1 );
		} );
	}
