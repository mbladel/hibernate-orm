...
@DomainModel( annotatedClasses = {
		AbstractTablePerClassVersionUpdateTest.Animal.class,
		AbstractTablePerClassVersionUpdateTest.Dog.class,
} )
@SessionFactory
public class AbstractTablePerClassVersionUpdateTest {
	@Test
	...
	public void testPessimisticIncrement(SessionFactoryScope scope) {
		...
		scope.inTransaction( session -> {
			final Dog dog = session.find( Dog.class, 2L, LockModeType.PESSIMISTIC_FORCE_INCREMENT );
			assertThat( dog.getVersion() ).isEqualTo( 1L );
	...
	public void setUp(SessionFactoryScope scope) {
		scope.inTransaction( session -> {
			...
			session.persist( new Dog( 2L, "dog_2" ) );
		} );
	}
