...
public class BasicValueHelper {
	...
	public static void bindTimeZoneStorage(
		...
		if ( columnAnn != null ) {
			...
			// columnAnn.getString( "name", property.getName() + "_tz" );
			final String name = columnAnn.getString( "name" );
			column.setName( isEmpty( name ) ? property.getName() + "_tz" : name );
			column.setSqlType( columnAnn.getString( nullIfEmpty( "columnDefinition" ) ) );
