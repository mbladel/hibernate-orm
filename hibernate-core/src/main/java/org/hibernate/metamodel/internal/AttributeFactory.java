...
//					);
	...
	private static final MemberResolver normalMemberResolver = (attributeContext, metadataContext) -> {
		...
		if ( Type.PersistenceType.EMBEDDABLE == persistenceType ) {
			...
			return resolveEntityMember( property, (AbstractIdentifiableType<?>) ownerType, metadataContext );
		}
		else {
	...
	private static Member resolveEntityMember(
			...
			AbstractIdentifiableType<?> ownerType,
			MetadataContext metadataContext) {
		final EntityPersister declaringEntityPersister = getDeclaringEntity( ownerType, metadataContext );
		...
		if ( attributeMapping == null ) {
			...
			return resolveVirtualIdentifierMember( property, ownerType, metadataContext );
		}
		else {
	...
	private static Member resolveMappedSuperclassMember(
		...
		if ( ownerType.getSubTypes().isEmpty() ) {
			...
			if ( persistenceType == Type.PersistenceType.ENTITY ) {
				return resolveEntityMember( property, (AbstractIdentifiableType<?>) subType, metadataContext );
			}
