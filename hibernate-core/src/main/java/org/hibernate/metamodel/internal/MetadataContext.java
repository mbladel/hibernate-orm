	...
	private void applyIdMetadata(PersistentClass persistentClass, IdentifiableDomainType<?> identifiableType) {
		if ( persistentClass.hasIdentifierProperty() ) {
			...
			if ( declaredIdentifierProperty != null ) {
				...
				final SingularPersistentAttribute<?, Object> genericEmbeddable = attributeFactory.buildIdAttribute(
						identifiableType,
						persistentClass.getIdentifierProperty()
				);
				//noinspection unchecked
				attributeContainer.getInFlightAccess().applyGenericEmbeddableAttribute( genericEmbeddable );
			}
		}
		else {
