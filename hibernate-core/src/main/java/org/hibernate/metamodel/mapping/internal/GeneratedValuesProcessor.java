...
public class GeneratedValuesProcessor {
	 ...
	 */
	public static Map<EventType, List<AttributeMapping>> getGeneratedAttributes(EntityMappingType entityDescriptor) {
		...
		final Map<EventType, List<AttributeMapping>> generatedValues = new HashMap<>();
		entityDescriptor.forEachAttributeMapping( mapping -> {
			...
			if ( generator != null && generator.generatedOnExecution() ) {
				for ( EventType eventType : generator.getEventTypes() ) {
					generatedValues.computeIfAbsent( eventType, k -> new ArrayList<>() ).add( mapping );
				}
			}
		} );
	...
	/**
	 * Utility method used to easily retrieve the result of {@link #getGeneratedAttributes(EntityMappingType)}
	 * and default to an empty list.
	 *
	 * @implNote The returned list must remain mutable
	 */
	public static List<AttributeMapping> getGeneratedAttributes(Map<EventType, List<AttributeMapping>> generatedAttributes, EventType timing) {
		final List<AttributeMapping> attributeMappings = generatedAttributes.get( timing );
		return attributeMappings == null ? new ArrayList<>() : attributeMappings;
	}
