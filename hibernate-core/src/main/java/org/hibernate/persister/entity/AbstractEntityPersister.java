...
//		return index == -1 ? 0 : getSubclassPropertyTableNumber( index );
	...
	protected String getPrunedTableExpression(
		...
		if ( containsNotNull ) {
			...
			discriminatorPredicate = frag.toFragmentString() + sb;
		}
		else {
			discriminatorPredicate = frag.toFragmentString();
		}
		return "(select * from " + getRootTableName() + " t where " + discriminatorPredicate + ")";
	}
