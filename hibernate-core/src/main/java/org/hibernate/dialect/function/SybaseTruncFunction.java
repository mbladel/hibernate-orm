...
/**
 * Custom {@link TruncFunction} for Sybase which uses the dialect-specific emulation function for datetimes
 *
...
public class SybaseTruncFunction extends TruncFunction {
	...
	protected <T> SelfRenderingSqmFunction<T> generateSqmFunctionExpression(
		...
		if ( arguments.size() == 2 && arguments.get( 1 ) instanceof SqmExtractUnit ) {
			// datetime truncation
			return new SybaseDateTruncEmulation( "convert", typeConfiguration ).generateSqmFunctionExpression(
					arguments,
					impliedResultType,
					queryEngine,
					typeConfiguration
