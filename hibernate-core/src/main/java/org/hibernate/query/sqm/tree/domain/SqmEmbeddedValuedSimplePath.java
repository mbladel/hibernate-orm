...
public class SqmEmbeddedValuedSimplePath<T>
	...
	public SqmPath<?> get(String attributeName) {
		...
		if ( pathSource instanceof CompositeSqmPathSource && ( (CompositeSqmPathSource<T>) pathSource ).isGeneric()
				&& ( lhsType = getLhs().getReferencedPathSource().getSqmPathType() ) instanceof ManagedDomainType ) {
